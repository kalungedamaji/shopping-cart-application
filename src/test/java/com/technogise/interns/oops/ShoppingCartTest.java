...
public class ShoppingCartTest {
  ...
  public void testCalculateTotalPriceOfCartIncludingTaxesWhenShoppingCartIsEmptyReturnsZero() {
    ...
    final BigDecimal EXPECTED_PRICE_OF_CART =
        BigDecimal.valueOf(0).setScale(TWO_DIGIT_PRECISION, RoundingMode.HALF_UP);
  ...
  public void testCalculateTotalPriceOfCartIncludingTaxesWhenMultipleCategoryOfProductsAreAdded() {
    ...
    BigDecimal actualTotalPriceOfCartIncludingTaxes =
        shoppingCart.calculateTotalPriceOfCartIncludingTaxes();
    assertEquals(
        EXPECTED_TOTAL_PRICE_OF_CART_INCLUDING_TAXES, actualTotalPriceOfCartIncludingTaxes);
  }
  ...
  public void testSalesTaxWhenDifferentProductsAreAddedToShoppingCart() {
    ...
    BigDecimal EXPECTED_TOTAL_DOVE_SOAP_SALES_TAX =
        BigDecimal.valueOf(NO_OF_DOVE_SOAP)
            .multiply(DOVE_SOAP_UNIT_PRICE)
    ...
    shoppingCart.addProducts(deoProduct, NO_OF_AXE_DEO);
    BigDecimal EXPECTED_TOTAL_AXE_DEO_SALES_TAX =
        BigDecimal.valueOf(NO_OF_AXE_DEO)
            .multiply(AXE_DEO_UNIT_PRICE)
            .multiply(salesTaxRate.divide(BigDecimal.valueOf(100)));

    BigDecimal EXPECTED_TOTAL_SALES_TAX =
        EXPECTED_TOTAL_DOVE_SOAP_SALES_TAX
            .add(EXPECTED_TOTAL_AXE_DEO_SALES_TAX)
  ...
  public void testUserHasAbilityToGiftOneItem() {
    ...
    assertThat(
        ...
        contains(hasProperty("userChoiceForGift", is(EXPECTED_USER_CHOICE_FOR_GIFT))));
  }
  ...
  public void testItemIsNotGiftable() {
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> shoppingCart.userOptedItemForGift(soapProduct));
    String actualMessage = exception.getMessage();
    assertTrue(actualMessage.contains(expectedMessage));
  }

  @Test
  public void testEmptyShoppingCartReturnsNoItemsInTheCartList() {
  ...
  @Test
  public void testGetOneItemFromShoppingCart() {
    final int NO_OF_DOVE_SOAP = 1;
    ...
    assertThat(actualItems, hasItem(hasProperty("name", is(SOAP_NAME))));
  }

  @Test
  public void testGetAllItemOfShoppingCart() {
    // given
    final int NO_OF_DOVE_SOAP = 1;
    final int NO_OF_AXE_DEO = 1;
    Product soapProduct = new Product(SOAP_NAME, DOVE_SOAP_UNIT_PRICE);
    ...
    assertThat(
        actualItems,
        hasItems(hasProperty("name", is(SOAP_NAME)), hasProperty("name", is(DEO_NAME))));
  }
}
